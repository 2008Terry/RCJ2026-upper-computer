...
std::uint16_t crc16Modbus(const std::string & data)
{
...
std::optional<ParsedReply> parseReplyLine(const std::string & line)
{
  ...
  const std::string command_text = line.substr(0, separator_pos);
  const std::string crc_text = line.substr(separator_pos + 1);
  const auto parsed_crc = parseCrcHex(crc_text);
  if (!parsed_crc.has_value() || crc16Modbus(command_text) != *parsed_crc) {
    return std::nullopt;
  }
...
public:
    ...
    }
  {
    ...
    RCLCPP_INFO(
      ...
      "resend_period_ms=%d, tick_period_ms=%d",
      port_.c_str(),
      baudrate_,
      timeout_sec_,
      resend_period_ms_,
