...
public:
    AmclFusionNode() : Node("amcl_fusion") {
        ...
        if (enable_localization_) {
            ...
            odom_sub_ =
                this->create_subscription<geometry_msgs::msg::PoseWithCovarianceStamped>(
                    odom_topic_,
                    10,
                    std::bind(&AmclFusionNode::odomCallback, this, std::placeholders::_1));
        ...
        parameter_callback_handle_ = this->add_on_set_parameters_callback(
            std::bind(&AmclFusionNode::handleParameterUpdates, this, std::placeholders::_1));

        RCLCPP_INFO(
            this->get_logger(),
            "amcl_fusion started. mask_topic='%s', yaw_topic='%s', odom_topic='%s', "
            "meters_per_pixel=%.6f, forward_axis='%s', left_axis='%s', max_points=%d, "
            "num_particles=%d, sigma_hit=%.3f, noise_xy=%.3f, noise_theta=%.3f, "
            "filter_period_ms=%d, publish_processing_time=%s, "
            "processing_time_topic='%s', enable_timing_log=%s, timing_log_interval=%d",
            mask_topic_.c_str(),
            yaw_topic_.c_str(),
            odom_topic_.c_str(),
            meters_per_pixel_,
...
private:
    ...
    rcl_interfaces::msg::SetParametersResult handleParameterUpdates(
        ...
        rcj_loc::ParticleFilterAmclFusionConfig candidate_filter_config = filter_config_;
        ...
        for (const auto &parameter : parameters) {
            ...
            if (name == "meters_per_pixel") {
                ...
                name == "map_topic" || name == "yaw_topic" || name == "odom_topic" ||
    ...
    void filterLoop() {
        ...
        if (!odom_initialized_) {
            ...
            pf_->predict(current_yaw_rad_, dx, dy);
        }
